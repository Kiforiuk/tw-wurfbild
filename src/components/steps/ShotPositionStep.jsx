          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 place-items-center">
            {positions.map((pos) => (
              <button
                ...
                className="bg-blue-600 hover:bg-blue-700 active:scale-95 p-6 md:p-8 rounded-xl transition transform duration-75 touch-none flex flex-col items-center justify-center min-h-[140px] md:min-h-[180px] w-full"
              >
                <div className="text-4xl md:text-5xl font-bold text-white leading-tight">{pos.label}</div>
                <div className="text-xs md:text-sm text-gray-200 mt-2 md:mt-3 text-center leading-snug">{pos.name}</div>
              </button>
            ))}
          </div>
        </div>
